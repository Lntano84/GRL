...
\begin{enumerate}
  ...
  \item \textbf{The corrected negative-share result is small.} Measured at $\mathrm{MC}=300$, the
        mean share over saturated cells is $0.5\%$, the worst cell in the entire sweep is $4.0\%$,
        and the mean share negative by more than two paired standard errors is $0.2\%$. The
        low-budget predecessor is excluded because its label noise is not a valid basis for this
        claim.
\end{enumerate}
